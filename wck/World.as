package wck {
	...
	public class World extends Scroller {
		...
		/**
		 * This event is fired for each timestep. Listeners with a priority higher than zero will be triggered
		 * before the time step. Priorities lower than zero will trigger after the timestep. 
		 */
		public static const TIME_STEP:String = 'onTimeStep';
		...
		public override function create():void {
			...
			b2world = new EventWorld(new V2(0, 0), allowSleep);
			listenWhileVisible(stage, Event.ENTER_FRAME, handleEnterFrame);
			listenWhileVisible(this, TIME_STEP, applyGravityToWorld, false, 10);
			listenWhileVisible(this, TIME_STEP, step);
			super.create();
		}
		...
		/**
		 * Dispatch a timestep event if we're not paused.
		 */
		public function handleEnterFrame(e:Event):void {
			if(!paused && b2world) {
				dispatchEvent(new Event(TIME_STEP));
			}
		}
